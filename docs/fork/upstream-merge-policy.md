...
cheap: a constant is importable anywhere, so no gate needs a prop threaded to it
or a hook, and every one of them is a single expression inside upstream code.
Flipping a surface back on is one edit in that file.

**Every gate is additive.** No gate introduces a prop, an effect, or state, and
none re-indents a block it did not add — where a conditional would have wrapped
existing JSX, the gate moved to a place that filters instead (`SurfaceMenuItem`
takes a `feature` prop; the command palette filters its finished action list).
That is deliberate: a re-indented block is what turns an upstream edit nearby
into a conflict, so keep any new gate to the same shape.
