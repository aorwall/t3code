...
| 3 | **Raw HTTP routes** — outside the typed API | 4 routes | *(no schema)* | [`apps/server/src/http.ts`][rawhttp] |
...
A server must attach its live event buffer *before* querying the snapshot, or an event
...
A healthy socket with a failed shell subscription is shown as *connected with a sync
error*, not as a reconnect.
...
| MCP | *(no schema here)* | agents ↔ server at `/mcp` ([`McpHttpServer.ts:214`][mcp]) |
