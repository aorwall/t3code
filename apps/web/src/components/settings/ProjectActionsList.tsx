...
export function ProjectActionsList({
  ...
  disabled: boolean;
  /**
   * Fork addition (Moatless). A git-synced workspace owns its actions in
   * .moatless/workspaces.json and reports them read-only, so the caller hides
   * the Edit button and leaves running them. Defaults to the upstream
   * behavior, which is always editable.
   */
  editable?: boolean;
