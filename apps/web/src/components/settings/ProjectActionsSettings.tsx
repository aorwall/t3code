...
import { portFromPreviewUrl } from "../../projectScripts";
...
export function ProjectActionsSettings() {
  ...
  // affordance and leave running them. Environment-level defaults are not a
  // workspace, so they keep upstream's always-editable behavior.
  const scriptsEditable = representativeMember
    ? (representativeMember.scriptsEditable ?? false)
    : true;
  ...
  return (
    <SettingsSection id="project-actions" title="Actions">
      <SettingsRow
        ...
        {...(scriptsEditable ? { onResetOverride: () => void persist(() => null) } : {})}
        control={
          <div className="flex flex-wrap items-center gap-1.5">
            {/* Fork: scriptsEditable gates the write affordances below. */}
