...
describe("searchSettings", () => {
  ...
  it("hides settings whose controls are unavailable", () => {
    const available = filterAvailableSettingsSearchItems({
      ...
      // Fork: gates the Version control page's Forgejo entry.
    ...
    const gatedIds = new Set<string>([
      ...
      // Fork: the Version control page's Forgejo entry.
  ...
  it("keeps the local toggle searchable without offering hidden host publishing controls", () => {
    const availability = {
      ...
      isWslSettingsRowVisible: false,
      hasThreadAutoSettlement: false,
      // Fork: the Forgejo availability filter, off for a deployment without it.
      forgejoEnabled: false,
    };
  ...
  it("routes browser recording quality to the browser settings section", () => {
  ...
  it.each([
    ...
    ["agent browser access", "agent-browser-access", "/settings/browser"],
