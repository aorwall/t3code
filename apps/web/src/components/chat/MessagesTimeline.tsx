...
// Fork: message-origin chip for messages that arrived through an adapter or another Task.
...
import {
  ...
  DownloadIcon,
  EyeIcon,
  // Fork: GitForkIcon for the fork-a-thread hover button.
  GitForkIcon,
  GlobeIcon,
  HammerIcon,
  HashIcon,
...
// Fork: a Moatless Task takes messages from Slack, GitHub, Linear, Telegram and
// other Tasks, so a message needs to say where it came from. Upstream's thread
// has one source, the composer, and renders no chip.
      ...
      <div className="relative min-w-0 px-1 py-0.5">
        ...
        {row.showAssistantMeta ? (
          <AssistantMessageMeta
            ...
            // Fork: fork-a-thread-from-the-chat hover action, beside the copy button.
            actions={<AssistantForkButton row={row} />}
          />
        ) : null}
      </div>
    </>
...
function AssistantMessageMeta({
  ...
  copyStreaming,
  alwaysVisible = false,
  // Fork: optional actions rendered beside the copy button (the fork-from-message button).
  actions,
}: {
  className?: string;
  message: ChatMessage;
