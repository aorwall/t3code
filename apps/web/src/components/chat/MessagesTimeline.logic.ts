...
export function deriveMessagesTimelineRows(input: {
  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    const messageTurnId = timelineEntry.message.turnId;
    // Fork: a message on a provably earlier moatless turn has already settled,
    // so it keeps its meta row and stays forkable while a later turn runs.
    // Moatless numbers task turns (task:1, task:3); a message turn that parses
    // below the running turn sits behind the backend's in-flight-turn check.
    // Upstream turns (a promptless restart) do not parse this way, so they keep
    // upstream's behaviour and stay folded into one active visual response.
    // This mirrors AssistantForkButton, which offers the action only when the
    // cut turn parses below the running turn.
    const messageTurnSettledBeforeRunning =
      messageTurnId !== null &&
      messageTurnId !== undefined &&
      unsettledTurnId !== null &&
      (() => {
        const cut = parseMoatlessTurnNumber(messageTurnId);
        const running = parseMoatlessTurnNumber(unsettledTurnId);
        return cut !== null && running !== null && cut < running;
      })();
    ...
    const assistantResponseStillInProgress =
      ...
      messageTurnId !== null &&
      messageTurnId !== undefined &&
      activeVisualResponseTurnIds.has(messageTurnId) &&
      !messageTurnSettledBeforeRunning;
