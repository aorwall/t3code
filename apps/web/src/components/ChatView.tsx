...
export default function ChatView(props: ChatViewProps) {
  ...
  // Fork: upstream derives configured preview URLs for local-server discovery,
  // which this fork deletes — a script in a remote sandbox has no localhost to
  // probe. See the project-script port field in docs/fork/inventory.json.
  useEffect(() => {
  ...
  const advertisedFileAttachmentBytes = supportsAttachmentUploads
  ...
  const autoUpdateEnvironments = useMemo(
    () =>
      // Fork: the auto-balance banner is the second route to the same
      // "Server update available" offer, so it takes the same gate as the
      // single-machine one — see FEATURES.serverUpdateBanner.
      FEATURES.serverUpdateBanner && automaticEnvironment
  ...
  // Fork: a path opened from a mention is the path someone wrote; the server
  // answers with the path it actually read. Retarget the open surface onto the
  // answer instead of opening a second one.
              ...
              <MessagesTimeline
                ...
                {...(!paintOnlyDisplayedTimeline
                  ? {
                      ...
                      // Fork: forking a thread from the chat hover action. Held
                      // paint-only timelines drop it with the other handlers.
                      onForkThread,
                      isForkingThread,
                    }
                  : {})}
      ...
      >
        <AlertDialogPopup>
          ...
          <AlertDialogFooter>
            ...
            {FEATURES.checkpointFileRestore && activeWorktreePath !== null ? (
              <Button
                variant="destructive"
                onClick={() => {
